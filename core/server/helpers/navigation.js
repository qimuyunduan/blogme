...
navigation = function (options) {
    ...
    if (!_.isObject(navigationData) || _.isFunction(navigationData)) {
        return errors.logAndThrowError('warnings.helpers.navigation.invalidData');
    }

    if (navigationData.filter(function (e) {
        return (_.isUndefined(e.label) || _.isUndefined(e.url));
    }).length > 0) {
        return errors.logAndThrowError('warnings.helpers.navigation.valuesMustBeDefined');
    }
    ...
    if (navigationData.filter(function (e) {
        ...
        return errors.logAndThrowError('warnings.helpers.navigation.valuesMustBeString');
    }
