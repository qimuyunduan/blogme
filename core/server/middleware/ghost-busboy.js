...
function ghostBusBoy(req, res, next) {
    ...
    busboy.on('file', function onFile(fieldname, file, filename, encoding, mimetype) {
        ...
        file.on('error', function onError(error) {
            console.log('Error', 'errors.middleware.ghostbusboy.fileUploadingError', error);
        });
    ...
    busboy.on('error', function onError(error) {
        console.log('Error', 'errors.middleware.ghostbusboy.somethingWentWrong', error);
        res.status(500).send({code: 500, message: 'errors.middleware.ghostbusboy.couldNotParseUpload'});
    });
