    ...
    return Promise.join(
        ...
        var payload = {
            mail: [{
                message: {
                    to: user.email,
                    subject: 'common.api.users.mail.invitedByName',
...
users = {
    ...
    read: function read(options) {
        ...
        return pipeline(tasks, options).then(function formatResponse(result) {
            ...
            return Promise.reject(new errors.NotFoundError('errors.api.users.userNotFound'));
        });
    },
            ...
            return canThis(options.context).edit.user(options.id).then(function () {
                ...
                return dataProvider.User.findOne(
                    ...
                    if (roleId !== contextRoleId && editedUserId === contextUser.id) {
                        return Promise.reject(new errors.NoPermissionError('errors.api.users.cannotChangeOwnRole'));
                    }
                        ...
                        if (contextUser.id !== owner.id) {
                            if (editedUserId === owner.id) {
                                if (owner.related('roles').at(0).id !== roleId) {
                                    return Promise.reject(new errors.NoPermissionError('errors.api.users.cannotChangeOwnersRole'));
                                }
                ...
                return errors.formatAndRejectAPIError(error, 'errors.api.users.noPermissionToEditUser');
            });
        }
    ...
    add: function add(object, options) {
        ...
        function handlePermissions(options) {
            ...
            return canThis(options.context).add.user(options.data).then(function () {
                if (newUser.roles && newUser.roles[0]) {
                    ...
                    return dataProvider.Role.findOne({id: roleId}).then(function (role) {
                        if (role.get('name') === 'Owner') {
                            return Promise.reject(new errors.NoPermissionError('errors.api.users.notAllowedToCreateOwner'));
                        }
                ...
                return errors.formatAndRejectAPIError(error, 'errors.api.users.noPermissionToAddUser');
            });
        }
        ...
        function doQuery(options) {
            ...
            if (newUser.email) {
                ...
                return Promise.reject(new errors.BadRequestError('errors.api.users.noEmailProvided'));
            }
            ...
            return dataProvider.User.getByEmail(
                ...
                if (!foundUser) {
                    ...
                    if (foundUser.get('status') === 'invited' || foundUser.get('status') === 'invited-pending') {
                        ...
                        return Promise.reject(new errors.BadRequestError('errors.api.users.userAlreadyRegistered'));
                    }
                }
                ...
                if (error && error.errorType === 'EmailError') {
                    error.message = 'errors.api.users.errorSendingEmail.error' + ' ' +
                        'errors.api.users.errorSendingEmail.help';
    ...
    destroy: function destroy(options) {
        ...
        function handlePermissions(options) {
            return canThis(options.context).destroy.user(options.id).then(function permissionGranted() {
                ...
                return errors.formatAndRejectAPIError(error, 'errors.api.users.noPermissionToDestroyUser');
            });
        }
    ...
    changePassword: function changePassword(object, options) {
        ...
        function handlePermissions(options) {
            return canThis(options.context).edit.user(options.data.password[0].user_id).then(function permissionGranted() {
                ...
                return errors.formatAndRejectAPIError(error, 'errors.api.users.noPermissionToChangeUsersPwd');
            });
        }
        ...
        return pipeline(tasks, object, options).then(function formatResponse() {
            return Promise.resolve({password: [{message: 'notices.api.users.pwdChangedSuccessfully'}]});
        });
    },
