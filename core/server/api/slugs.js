    ...
    generate: function (options) {
        ...
        function checkAllowedTypes(options) {
            if (allowedTypes[options.type] === undefined) {
                return Promise.reject(new errors.BadRequestError('errors.api.slugs.unknownSlugType'));
            }
            return options;
        }
        ...
        return pipeline(tasks, options).then(function (slug) {
            if (!slug) {
                return Promise.reject(new errors.InternalServerError('errors.api.slugs.couldNotGenerateSlug'));
            }
