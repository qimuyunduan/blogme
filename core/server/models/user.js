...
User = ghostBookshelf.Model.extend({
    ...
    contextUser: function contextUser(options) {
        ...
        if (options.context && options.context.user) {
            ...
            errors.logAndThrowError(new errors.NotFoundError('errors.models.user.missingContext'));
        }
    },
    ...
    */
    ...
    permissible: function permissible(userModelOrId, action, context, loadedPermissions, hasUserPermission, hasAppPermission) {
        ...
        if (action === 'destroy') {
            ...
            if (loadedPermissions.user && userModel.hasRole('Owner')) {
                return Promise.reject(new errors.NoPermissionError('errors.models.user.notEnoughPermission'));
            }
    ...
    transferOwnership: function transferOwnership(object, options) {
        ...
        .then(function then(results) {
            ...
            if (!_.any(currentRoles, {id: ownerRole.id})) {
                return Promise.reject(new errors.NoPermissionError('errors.models.user.onlyOwnerCanTransferOwnerRole'));
            }
